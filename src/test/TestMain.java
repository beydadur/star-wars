...
public class TestMain {
	...
	public static void boostHealths(ArrayList<ForceWielder> allCharacters) {
		for (int i=0; i < allCharacters.size(); i++) {
			if (allCharacters.get(i) instanceof Jedi) {
				((Jedi) allCharacters.get(i)).boostHealth();
			}
			
			if (allCharacters.get(i) instanceof Master) {
			((Master) allCharacters.get(i)).boostHealth();
